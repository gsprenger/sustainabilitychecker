class window.Choice 
  @initRadio: (el) ->
    sectionID = $(el).closest('.section').attr('id')
    $(el).find('[data-cv-value]').each (i, radio) ->
      ...
      for val in Progression.values
        if val.name == sectionID && val.value == value
          $(radio).addClass('active')
          break
      # attach a click event to the radio element
      $(radio).on 'click', ->
        $('.section[id='+sectionID+'] .choice-grid-radio.active').removeClass('active')
        $(radio).addClass('active')
        Progression.current = sectionID
        Progression.addToValues sectionID, value
        Progression.save()
